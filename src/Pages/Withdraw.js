...
const Withdraw = () => {
  ...
  let handleWihdrawal = (event) => {
    event.preventDefault();
    toast.success('you have successfully place a withdrawer')
  }
        ...
        <main className='flex h-[100vh] flex-col w-full px-[10px] md:px-[20px] py-[10px] items-center justify-center overflow-hidden'>
          <section className='w-full h-[100vh] flex flex-col gap-3 items-center justify-center md:mt-14'>
            <form className='md:w-3/6 w-[90%] flex flex-col rounded-md items-center justify-center p-4 bg-blue-text-color gap-3'>
              ...
              <input className='md:w-[80%] w-full h-[55px] bg-logo-color text-xl text-white font-sans font-bold capitalize p-3 rounded-lg cursor-pointer hover:scale-95 transition-all duration-700' type='submit' name='submit' onClick={handleWihdrawal}/>
            </form>
          </section>
        </main>
      </aside>
